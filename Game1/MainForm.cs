...
namespace Game1
{
    ...
    public partial class MainForm : Form
    {
        ...
        private void PlayerTimer_Tick(object sender, EventArgs e)
        {
            ...
            if (!game.Iterate)
            {
                MessageBox.Show(game.EndGame() ? "Gagné!" : "Perdu!");
        ...
        private Bitmap DrawGame()
        {
            ...
            for(int i = 0; i < 3; i++)
            {
                for(int ii = 0; ii < 3; ii++)
                {
                    ...
                    {
                        case "Ronds":
                            g.FillEllipse(brush, new Rectangle(i * (size + 4), ii * (size + 4), size, size));
                            break;
                        case "Rectangles":
                            g.FillRectangle(brush, new Rectangle(i * (size + 4), ii * (size + 4), size, size));
                            break;
                        case "Arrondi":
                            g.FillPath(brush, RoundRect(new Rectangle(i * (size + 4), ii * (size + 4), size, size)));
                            break;
                    }
                }
            }
        ...
        private Bitmap DrawGameWithSelectedTile(int x, int y)
        {
            ...
            {
                case "Ronds":
                    g.FillEllipse(white, new Rectangle(x * (size + 4) + 2, y * (size + 4) + 2, size - 4, size - 4));
                    break;
                case "Rectangles":
                    g.FillRectangle(white, new Rectangle(x * (size + 4) + 2, y * (size + 4) + 2, size - 4, size - 4));
                    break;
                case "Arrondi":
                    g.FillPath(white, RoundRect(new Rectangle(x * (size + 4) + 2, y * (size + 4) + 2, size - 4, size - 4)));
                    break;
            }
            g.Dispose();
            return bmp;
